...
var nlc2_rpc = new RPC.CoinRPC(config.nodeServer.nlc2.rpc, {auth: config.nodeServer.nlc2.rpcAuth});
var service = {};
//  _       _ 	  ___        _      _______   __      __   _______   _______
// | |\    | |  / __ \      | |    |__  ___| |   \   /  | |__   __| |__   __|  
// | |\ \  | | | |  | | ___ | |       | |    | |\ \/ /| |    | |       | |  
// | | \ \ | | | |  | ||___|| |       | |    | | \ /  | |    | |       | |
// | |  \ \| | | |__| |     | |___  __| |__  | |      | |  __| |__     | |
// |_|   \ |_|  \____/      |_____||_______| |_|      |_| |_______|    |_|
/**
 * Service methods 
 */
...
function newAccount() {
	...
	nlc2_rpc.call('getnewaddress', []).then(address => {
		res.address = address;
		return nlc2_rpc.call('dumpprivkey', [res.address]);
	}).then(pk => {
		res.pk = pk;
		return nlc2_rpc.call('validateaddress', [res.address]);
	}).then(validation => {
...
function getBalance(address) {
	...
	nlc2_rpc.call('listunspent', []).then(txs => {
		let balance = 0;
...
function transfer(pk, fromAddress, toAddress, amount) {
	...
	nlc2_rpc.call('settxfee', [fee]).then(() => {
		return nlc2_rpc.call('listunspent', []);
	}).then(txs => {
		// let untx = [];
		let tx_obj = [];
		let addr_amount_obj = {};
		...
		if (Array.isArray(txs) && txs.length > 0) {
			...
			if (balance < amount + fee) {
				...
				return nlc2_rpc.call('createrawtransaction', [tx_obj, addr_amount_obj]);
			}
		} else {
			throw {message: 'Insufficient funds!'};
		}
	}).then(rawtx => {
		return nlc2_rpc.call('signrawtransaction', [rawtx, [], [pk]])
	}).then(signedtx => {
		if (signedtx.complete === 0) throw {message: 'Failed to sign the Raw Transaction.'}; 
		return nlc2_rpc.call('sendrawtransaction', [signedtx.hex])
	}).then(sendtxid => {
...
function listTransactionsByAddress(address) {
	let deferred = Q.defer();
	let return_txs = [];
	nlc2_rpc.call('listtransactions', ["", 5000000000000]).then((txs) => {
		if (Array.isArray(txs) && txs.length > 0) {
			_.each(txs, (tx) => {
...
function getFee() {
	var reslt = 0;
	let deferred = Q.defer();
	nlc2_rpc.call('getinfo',[]).then((txs) =>{
		console.log(txs.paytxfee)
		reslt =txs.paytxfee;
	    deferred.resolve(reslt);
}).catch(err => {
	deferred.reject(err.message);
});;
return deferred.promise;
...
function importPrivkey(pk) {
	...
	nlc2_rpc.call('importprivkey', [pk, "", false]).then((result) => {
...
function validateAddress(address) {
	...
	nlc2_rpc.call('validateaddress', [address]).then((result) => {
		response = result;
		return nlc2_rpc.call('dumpprivkey', [address]);
	}).then((result) => {
...
function checkStatus() {
	...
	nlc2_rpc.call('getinfo', []).then((result) => {
		console.log(result)
