...
var vites_rpc =  new RPC.CoinRPC(config.nodeServer.vites.rpc, {auth: config.nodeServer.vites.rpcAuth});
// __       __  _________  ________  ________   _______
...
async function newAccount() {
	...
	await vites_rpc.call('getnewaddress', []).then(address => {
		res.address = address;
		return vites_rpc.call('dumpprivkey', [res.address]);
	}).then(pk => {
		res.pk = pk;
		return vites_rpc.call('validateaddress', [res.address]);
	}).then(validation => {
...
function getBalance(address) {
	...
	vites_rpc.call('listunspent', []).then(txs => {
		let balance = 0;
...
async function getFee() {
        let resp="";
        await vites_rpc.call("getinfo",[]).then(res=>{
            resp +=res.paytxfee;        
        })
        return resp;
}
...
async function transfer(pk, fromAddress, toAddress, amount) {
	...
	vites_rpc.call('settxfee', [fee]).then(() => {
		return vites_rpc.call('listunspent', []);
	}).then(txs => {
		// let untx = [];
		let tx_obj = [];
		let addr_amount_obj = {};
		...
		if (Array.isArray(txs) && txs.length > 0) {
			...
			if (balance < amount + fee) {
				...
				return vites_rpc.call('createrawtransaction', [tx_obj, addr_amount_obj]);
			}
		} else {
			throw {message: 'Insufficient funds!'};
		}
	}).then(rawtx => {
		return vites_rpc.call('signrawtransactionwithkey', [rawtx,[pk]])
	}).then(signedtx => {
		if (signedtx.complete === 0) throw {message: 'Failed to sign the Raw Transaction.'}; 
		return vites_rpc.call('sendrawtransaction', [signedtx.hex])
	}).then(sendtxid => {
...
function listTransactionsByAddress(address) {
	let deferred = Q.defer();
    let return_txs = [];
	vites_rpc.call('listtransactions',["*",5000,0]).then((txs) => {
		if (Array.isArray(txs) && txs.length > 0) {
			_.each(txs, (tx) => {
...
function importPrivkey(pk) {
	...
	vites_rpc.call('importprivkey', [pk, "", false]).then((result) => {
...
function validateAddress(address) {
	...
	vites_rpc.call('validateaddress', [address]).then((result) => {
		response = result;
		return vites_rpc.call('dumpprivkey', [address]);
	}).then((result) => {
...
function checkStatus() {
	...
	vites_rpc.call('getinfo', []).then((result) => {
